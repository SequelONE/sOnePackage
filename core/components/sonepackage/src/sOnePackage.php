...
class sOnePackage
{
     ...
     * @var string
     */
    protected $namespace = 'sonepackage';
    /**
    ...
    /**
     * The main constructor for sOnePackage. This doesn't hardcode the instance to the modX class as that might change in
    ...
    public function __construct($instance, array $config = array())
    {
        ...
        if ($this->loadPackage && ($this->namespace !== 'sonepackage')) {
            $this->modx->addPackage($this->namespace, $this->config['model_path']);
        }
        if ($this->loadLexicon && ($this->namespace !== 'sonepackage')) {
